...
if [ ! -z "$GIT_STATUS_DIRTY" ]; then
  ...
  #git add . && git commit -a -m "update origin after merge upstream "
  git add . && git commit -a -m "./dq_push_sync_fork_github.sh git@github.com:ducquoc/hook-check-executor.git"
